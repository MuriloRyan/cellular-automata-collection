...
class ForestFire:
    def __init__(self, horizontal_size: int = 64, vertical_size: int = 64, seed: int | None = None) -> None:
        ...
        self.grid = [[0] * horizontal_size for _ in range(vertical_size)]
        self.next_grid = [[0] * horizontal_size for _ in range(vertical_size)]
    ...
    def spread(self, x: int, y: int):
        ...
        nb4, main, nb5 = self.get_cell(x-1, y), self.get_cell(x, y), self.get_cell(x +1, y)
